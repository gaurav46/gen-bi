...
- [x] Each widget card on the dashboard detail page shows a pencil icon button
- [x] Clicking the pencil icon opens a dialog with the current title in an editable text field
- [x] The dialog lists each measure column with an editable label field, pre-filled with the custom label (if set) or the original column name
- [x] Saving the dialog calls PATCH and updates the widget in-place (no full page reload)
- [x] Cancelling the dialog discards changes
- [x] Chart legend displays custom labels instead of raw column names when `legendLabels` is set
- [x] When `legendLabels` is not set or a column has no custom label, the original column name is used
